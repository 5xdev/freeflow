...
# Separate binary because a Swift executable target can only hold one @main.
$(TRANSCRIPTION_TEST_RUNNER): Sources/TranscriptionProvider.swift Sources/TranscriptionService.swift Sources/LLMAPITransport.swift Tests/TranscriptionProviderTests.swift
	@mkdir -p "$(BUILD_DIR)"
	swiftc \
		-parse-as-library \
		...
		-sdk $(shell xcrun --show-sdk-path) \
		-target $(ARCH)-apple-macosx13.0 \
		Sources/TranscriptionProvider.swift Sources/TranscriptionService.swift Sources/LLMAPITransport.swift Tests/TranscriptionProviderTests.swift
